...
const Home = () => {
  ...
  useEffect(()=>{
    ...
    .then(res => res.json())
    .then((ty)=>{setcategories(ty)});
  },[])
  

  useEffect(()=>{
    fetch('https://dummyjson.com/products?limit=8&skip=32')
    .then(res => res.json())
    .then((ly)=>{setlimit(ly.products)});
  },[])

  useEffect(()=>{
    fetch('https://dummyjson.com/products?limit=8')
    .then(res => res.json())
    .then((ry)=>{setrecentProduct(ry.products)});
  },[])
  ...
  {categories.map((k)=>(
   ...
   <div className="col-lg-3 col-md-4 col-sm-6 pb-1">
     <Link className="text-decoration-none " to={`/searchProductByCategory/${k.slug}`}>
       <div className="cat-item  d-flex align-items-center mb-4">
         <div className="overflow-hidden" style={{width: 100, height: 100}}>
           {/* <img className="img-fluid" src="assets/img/cat-1.jpg" alt /> */}
           <i className='iconSize' class='bx bxs-shopping-bags'></i>
         </div>
         <div className="flex-fill pl-3">
           <h6>{k.name}</h6>
           <small className="text-body">{k.slug}</small>
         </div>
       </div>
     </Link>
   </div>
    ...
    <div className="col-lg-3 col-md-4 col-sm-6 pb-1">
        <div className="product-item bg-light mb-4">
          <div className="product-img position-relative overflow-hidden">
            <img className="img-fluid w-100" src={n.thumbnail} alt />
          ...
          <div className="text-center py-4">
            <a className="h6 text-decoration-none text-truncate" href>{n.title}</a>
            <div className="d-flex align-items-center justify-content-center mt-2">
              <h5>${n.price}</h5><h6 className="text-muted ml-2"><del>${n.price}</del></h6>
            </div>
  ...
  <div className="container-fluid pt-5 pb-3">
    ...
    <div className="row px-xl-5">
      ...
      {recentProduct.map((r)=>(
        ...
        <div className="product-item bg-light mb-4">
          <div className="product-img position-relative overflow-hidden">
            <img className="img-fluid w-100" src={r.thumbnail} alt />
            <div className="product-action">
              <a className="btn btn-outline-dark btn-square" href><i className="fa fa-shopping-cart" /></a>
              <a className="btn btn-outline-dark btn-square" href><i className="far fa-heart" /></a>
              <a className="btn btn-outline-dark btn-square" href><i className="fa fa-sync-alt" /></a>
          ...
          <div className="text-center py-4">
            <a className="h6 text-decoration-none text-truncate" href>{r.title}</a>
            <div className="d-flex align-items-center justify-content-center mt-2">
              <h5>{r.price}</h5><h6 className="text-muted ml-2"><del>{r.price}</del></h6>
            </div>
            <div className="d-flex align-items-center justify-content-center mb-1">
              <small className="fa fa-star text-primary mr-1" />
              <small className="fa fa-star text-primary mr-1" />
  ...
  <div className="container-fluid py-5">
    <div className="row px-xl-5 col-sm-row">
      <div className="col">
        <div className=" d-flex flex-col owl-carousel vendor-carousel">
